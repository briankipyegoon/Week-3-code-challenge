...
function createSpan(r, c, letter = '') {
    ...
    span.innerHTML = `${letter}<sup style="font-size:10px;">${letterScores[letter] || ''}</sup>`;
    return span;
}
...
function calculateScoreCAT() {
    ...
    scoreDisplay.innerText = `Score for CAT: ${score}`;
}
...
function calculateScoreTASK() {
   ...
   const letters = ['A', 'S', 'K'];
                ...
                // Check if the letter is on a Triple Letter
    

    letters.forEach(letter => {
        const positions = targetPositionsTASK.map(pos => `${pos.row},${pos.col}`);
        positions.forEach(pos => {
            if (bonuspoints.DL.includes(pos)) {
                score += (letterScores[letter] || 0) * 2; // Double Letter Score
